...
def agregarRutina(mysql, request):       
    ...
    cur= mysql.cursor()
    cur.execute('SELECT rutina.nombre FROM rutina JOIN usuario_rutina  ON rutina.id_rutina=usuario_rutina.id_rutina JOIN usuario ON usuario.id_usuario=usuario_rutina.id_usuario WHERE rutina.nombre=%s and usuario.id_usuario = %s',(rutina, session['username'],))
