...
def registrarHistorial(mysql, idrutina, ej, pesos, repeticiones, descansos):
    cur= mysql.cursor()
    cur.execute('INSERT into historial_rutina(id_rutina, id_ejerc, fecha_realizacion, peso, repeticiones, descanso) VALUES (%(int)s, %s , %s, %s, %s, %s)', (idrutina, ej,datetime.today().strftime('%Y-%m-%d') , pesos, repeticiones, descansos,))
    mysql.commit()
